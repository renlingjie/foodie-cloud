...
public class ItemsEsServiceImpl implements ItemsEsService {
    ...
    public PagedGridResult searchItems(String keywords, String sort, Integer pageIndex, Integer pageSize) {
        ...
        // 根据查询对象得到查询结果(高亮版)
        AggregatedPage<Items> itemsList = esTemplate.queryForPage(searchQuery, Items.class, new SearchResultMapper() {
            @Override
            public <T> AggregatedPage<T> mapResults(SearchResponse response, Class<T> clazz, Pageable pageable) {
                List<Items> chunk = new ArrayList<>();
                // 说明：高版本ES对应的SearchHit会更简单，它有方法searchHit.getContent()直接拿到对象，不用像这样先拿到searchHit查到的ES数据的json，再转对象
                for (SearchHit searchHit : response.getHits()) {
                    ...
                    items = JsonUtils.jsonToPojo(sourceAsString, Items.class);
                    // 之后将高亮的itemName重新赋值给上面的对象
                    HighlightField itemNameHF = searchHit.getHighlightFields().get("itemName");
                    if (itemNameHF != null) {
                        items.setItemName(itemNameHF.fragments()[0].toString());
                    }
        ...
        gird.setRows(itemsList.getContent());//每行显示的内容
        int totalPage;
